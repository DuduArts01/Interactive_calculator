        ...
        while running:
            ...
            if self.send.action:
                ...
                if logic.is_correct(answer_user):
                    ...
                    ### MODIFICAÇÃO INÍCIO: Mensagem de Acerto
                    
                    # 1. Mensagem de acerto
                    self.background_green.draw()
                    self.PVsComputerTitle.draw(self.screen)
                    correct_msg = Font("Resposta Correta! +5s no tempo!", "Arial", 60)
                    correct_msg.draw(self.screen, y=self.height / 2.2)
                    pygame.display.update()
                    sleep(2)
                    
                    # 2. Mensagem de retirar o cartão
                    self.background_green.draw()
                    self.PVsComputerTitle.draw(self.screen)
                    remove_card_msg = Font("Retire o cartão de reposta!", "Arial", 60)
                    remove_card_msg.draw(self.screen, y=self.height / 2.2)
                    pygame.display.update()
                    sleep(2) # Pausa de 2 segundos
                    
                    ### MODIFICAÇÃO FIM: Mensagem de Acerto
                    ...
                    ### MODIFICAÇÃO INÍCIO: Mensagem de Erro
                    
                    # Mensagem de retirar o cartão
                    self.background_green.draw()
                    self.PVsComputerTitle.draw(self.screen)
                    remove_card_msg = Font("Retire o cartão de reposta!", "Arial", 60)
                    remove_card_msg.draw(self.screen, y=self.height / 2.2)
                    ...
                    ### MODIFICAÇÃO FIM: Mensagem de Erro
